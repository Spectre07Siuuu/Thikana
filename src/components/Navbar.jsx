    ...
    <header className={`fixed top-4 left-0 right-0 z-50 transition-all duration-300 pointer-events-none px-4 sm:px-6 lg:px-8`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="pointer-events-auto">
          <Link to="/" className={`flex items-center gap-2.5 group rounded-full px-4 py-2 transition-colors ${scrolled ? 'liquid-glass-invert text-theme-text' : 'liquid-glass text-white'}`} aria-label="Thikana home">
            <Logo size={24} />
        ...
        {/* ── Desktop Nav ─── */}
        <nav className={`hidden md:flex items-center gap-1 rounded-full px-1.5 py-1.5 pointer-events-auto transition-colors ${scrolled ? 'liquid-glass-invert' : 'liquid-glass'}`} aria-label="Main navigation">
        ...
        ${isActive ? (scrolled ? 'bg-black/10 dark:bg-white/10 text-theme-text' : 'bg-white/20 text-white') : (scrolled ? 'text-theme-text hover:bg-black/5 dark:hover:bg-white/5' : 'text-white/90 hover:bg-white/10 hover:text-white')}`}>
                {label}
              </a>
            ) : (
        ...
        {/* ── Right Actions ─── */}
        <div className={`flex items-center gap-2 pointer-events-auto rounded-full px-2 py-1.5 transition-colors ${scrolled ? 'liquid-glass-invert text-theme-text' : 'liquid-glass text-white'}`}>
          ...
          <button
            ...
            className={`p-1.5 rounded-full transition-all duration-200 ${scrolled ? 'hover:bg-black/5 dark:hover:bg-white/5' : 'hover:bg-white/10'}`}
          >
            {dark
            ...
            <>
              ...
              {!user?.is_admin && user?.nid_verified === 1 && (
                <Link to="/messages" aria-label="Messages"
                  className={`hidden sm:flex p-1.5 rounded-full transition-all duration-200 relative ${scrolled ? 'hover:bg-black/5 dark:hover:bg-white/5' : 'hover:bg-white/10'}`}>
          ...
          {user ? (
            <div ref={profileRef} className="relative">
              <button
                ...
                aria-haspopup="true"
                className={`flex items-center gap-2 pl-1 pr-3 py-1 rounded-full border border-transparent transition-all duration-200 ${scrolled ? 'hover:bg-black/5 dark:hover:bg-white/5' : 'hover:bg-white/10'}`}
              >
        ...
        ${profileOpen ? 'rotate-180' : ''}`} />
              ...
              {/* Dropdown */}
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-64 dropdown-panel py-2 animate-fade-in !text-theme-text" role="menu">
                  {/* User Info */}
           ...
           hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors font-medium">
            ...
            <div className="hidden sm:flex items-center gap-1 ml-1">
              <Link to="/login"
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${scrolled ? 'hover:bg-black/5 dark:hover:bg-white/5' : 'hover:bg-white/10 text-white'}`}>
                Sign In
              </Link>
              <Link to="/signup" className={`transition-opacity font-semibold text-sm py-1.5 px-4 rounded-full ${scrolled ? 'bg-theme-text text-theme-bg dark:bg-white dark:text-black hover:opacity-90' : 'bg-white text-black hover:opacity-90'}`}>
                Sign Up
              </Link>
            </div>
          )}
          ...
          <button id="mobile-menu-toggle" onClick={() => setMobileOpen(v => !v)}
            ...
            className={`md:hidden p-1.5 rounded-full transition-colors ${scrolled ? 'hover:bg-black/5 dark:hover:bg-white/5' : 'hover:bg-white/10 text-white'}`}>
