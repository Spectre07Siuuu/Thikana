    ...
    <>
      ...
      {success && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl animate-modal-in border border-emerald-100 dark:border-emerald-900/30">
            <div className="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mx-auto mb-6">
              <CheckCircle size={40} className="text-emerald-500" />
            </div>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Upload Successful!</h3>
            <p className="text-gray-500 dark:text-gray-400">Your product has been submitted and is pending admin review.</p>
          </div>
        </div>
      )}
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 animate-fade-in">
          ...
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Link to="/profile" className="p-2 rounded-lg text-gray-400 hover:text-orange-500 hover:bg-white dark:hover:bg-gray-900 border border-transparent transition-all">
                <ArrowLeft size={18} />
              </Link>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Upload New Product</h1>
            </div>
            ...
            <div className="p-6 sm:p-10">
              ...
              {step === 1 && (
                <div className="animate-fade-in">
                  <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-2">What are you listing?</h2>
                  <p className="text-gray-500 text-sm mb-6">Choose the category that best fits your product.</p>
                        ...
                        <button key={cat.id} onClick={() => { setCategory(cat.id); setError(''); }}
                          className={`text-left p-5 rounded-2xl border-2 transition-all duration-200 flex items-start gap-4
                            ${isSelected 
                              ...
                              : 'border-gray-100 dark:border-gray-800 hover:border-orange-200 dark:hover:border-orange-900'
                            }`}
                        >
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 transition-colors
                            ${isSelected ? 'bg-orange-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>
                            <cat.icon size={20} />
                          </div>
                          <div>
                            <h3 className={`font-bold text-base mb-1 ${isSelected ? 'text-orange-700 dark:text-orange-400' : 'text-gray-900 dark:text-white'}`}>{cat.label}</h3>
                ...
                <div className="animate-fade-in">
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
                      <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4">Specific Details</h3>
                      ...
                      {(category === 'house_sell' || category === 'house_rent') && (
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                          ...
                          <div>
                            <label className="block text-xs font-medium text-gray-500 mb-1">Size (sqft)</label>
                          ...
                          {category === 'house_rent' && (
                            <div>
                              <label className="block text-xs font-medium text-gray-500 mb-1">Available For</label>
              ...
              {step === 3 && (
                <div className="animate-fade-in">
                  <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Upload Photos</h2>
                  <p className="text-gray-500 text-sm mb-6">Upload up to 5 clear photos. The first photo will be the main cover.</p>
                  
                  <label className="w-full h-40 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-700 bg-gray-50 hover:bg-orange-50/50 dark:bg-gray-800/30 dark:hover:bg-orange-950/10 flex flex-col items-center justify-center cursor-pointer transition-colors group mb-6">
                    <div className="w-12 h-12 rounded-full bg-white dark:bg-gray-800 shadow-sm flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                      <ImageIcon size={20} className="text-gray-400 group-hover:text-orange-500 transition-colors" />
                    </div>
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Click to upload photos</p>
                    <p className="text-xs text-gray-400 mt-1">JPEG, PNG, WEBP (Max 5)</p>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                      {images.map((img, i) => (
                        <div key={i} className="relative aspect-square rounded-xl overflow-hidden group shadow-sm bg-gray-100 dark:bg-gray-800">
                          ...
                          {i === 0 && (
                            <span className="absolute bottom-0 left-0 right-0 bg-orange-500/90 text-white text-[10px] font-bold text-center py-0.5">COVER</span>
                          )}
                        </div>
                      ))}
                    </div>
